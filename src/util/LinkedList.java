...
public class LinkedList<E extends Comparable<E>>
    ...
    */
    ...
    public E remove(int k) {
        ...
        E removedData;
        if (k == 0) {
            removedData = first.element;
            first = first.next;
            if (size == 1) {
                last = null; // If there was only one element
            }
        } else {
            Node<E> temp = first;
            for (int i = 0; i < k - 1; i++) {
            ...
            removedData = temp.next.element;
            temp.next = temp.next.next;
            if (k == size - 1) {
                last = temp; // Update last if removing the last element
            }
        }
        size--;
        return removedData;
